ssh root@157.245.8.48 "cd ~/taranveer-singh.github.io && python3 -c '
import sys
...
for log_file, identifier in log_files:
    last_exit_time = {}
    loss_bucket = 0.0
    print(f\"--- Parsing {log_file} for 2026-02-27 ---\")
    trades_found = False
    try:
        with open(log_file, \"r\") as f:
            for line_idx, line in enumerate(f):
                if \"2026-02-27\" not in line:
                    continue
                ...
                if side in [\"SELL\", \"COVER\"]:
                    ...
                    if per_share_pnl < -0.0001:
                        ...
                        if loss_bucket >= 0.02:
                            print(f\"{line} -> !!! PENALTY BOX LOGIC MET on line {line_idx+1}: {symbol} loss of ${abs(per_share_pnl):.4f} pushed bucket to ${loss_bucket:.4f}\")
                            loss_bucket = 0.0 
                    elif per_share_pnl > 0.0001 or abs(per_share_pnl) <= 0.0001:
                        loss_bucket = 0.0
                        
        if not trades_found:
            print(\"No trades found for 2026-02-27.\")
