...
export function PatternLabPage() {
  ...
  const { data: equityCurve } = useQuery({
    queryKey: ['pattern-equity-paper', range],
    queryFn: () => patternService.getEquityCurve(range, 'paper').then((r) => r.data.points),
    refetchInterval: 10000,
  })

  const { data: performance } = useQuery({
    queryKey: ['pattern-performance-paper'],
    queryFn: () => patternService.getPerformance('paper').then((r) => r.data.rows),
    refetchInterval: 10000,
  })
  ...
  return (
    <div style={{ padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <span style={{
          fontSize: '0.6rem', fontWeight: 700, color: BLUE,
