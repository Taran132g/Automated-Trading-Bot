#!/bin/bash
# manager.sh - Main entry point for starting and stopping the trading bot infrastructure
...
start_backend() {
    if pgrep -f "$LOOP_SCRIPT" > /dev/null; then
        ...
        # Ensure kill switch is removed before starting
...
stop_backend() {
    ...
    # 1. Activate kill switch for the loop
    touch kill_switch.flag
    
    # 2. Kill the manager loop itself
    ...
    echo "Backend Stopped (Kill Switch Active)."
}
